...
(provide repository? cons_repositorio) 
(provide rep_get_workspace rep_set_workspace) 
(provide rep_get_index rep_set_index)
(provide rep_get_local_rep  rep_set_local_rep)
(provide rep_get_remote_rep rep_set_remote_rep)
...
(define repository? (lambda (repositorio)
            (if (and (list? repositorio)(rama? (get-n-lista repositorio 0))(workspace? (get-n-lista repositorio 1))(index? (get-n-lista repositorio 2))(local_repository? (get-n-lista repositorio 3))
                     (remote_repository? (get-n-lista repositorio 4))(funciones? (get-n-lista repositorio 5)))
                #t
                #f
            )
      )
...
(define rep_get_ramificacion (lambda (repositorio)(if (repository? repositorio) (get-n-lista repositorio 0) null)))
(define rep_get_workspace (lambda (repositorio) (if (repository? repositorio) (get-n-lista repositorio 1) null)))
(define rep_get_index (lambda (repositorio)(if (repository? repositorio) (get-n-lista repositorio 2) null)))
(define rep_get_local_rep (lambda (repositorio)(if (repository? repositorio) (get-n-lista repositorio 3) null)))
